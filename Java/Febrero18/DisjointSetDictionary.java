...
public class DisjointSetDictionary<T extends Comparable<? super T>> implements DisjointSet<T> {
    ...
    public boolean areConnected(T elem1, T elem2) {
        if (root(elem1) == null || root(elem2) == null) {
            return false;
        } else return root(elem1)==root(elem2) ;
    }
    ...
    public List<T> kind(T elem) {
        ...
        if (isElem(elem)) {
            T val = dic.valueOf(elem);
            Iterable<Tuple2<T, T>> t = dic.keysValues();
            Iterator<Tuple2<T, T>> t2 = t.iterator();
            Tuple2<T, T> tp;
            while (t2.hasNext()) {
                tp = t2.next();
                if (tp._2() == val) {
    ...
    public void union(T elem1, T elem2) {
        ...
        if (isElem(elem1) && isElem(elem2)) {
            if (elem1.compareTo(elem2) < 0) {
                ...
                for (T elem : lista) {
                    dic.insert(elem, elem1);
                }
            } else {
                lista = kind(elem1);
                dic.insert(elem1, elem2);
                for (T elem : lista) {
                    dic.insert(elem, elem2);
                }
            }
