...
simplify (Unif a) (Unif b)
  | a == b = (Unif a)
  | otherwise = Node (Unif a) (Unif b)
...
get i (V tam arbol)
 | i < 0 || i>= tam = error "se sale de los indices"
 | otherwise = getAux i (V tam arbol)
  where
   getAux i (V tam (Unif x)) = x
   getAux i (V tam (Node lt rt))
    | i < mitad = getAux i (V mitad lt)
	| otherwise = getAux (i-mitad) (V mitad rt)
	 where
	  mitad = div tam 2

-------------------------------------------------------------------------------
-- | Exercise e.
get :: Int -> a -> Vector a -> Vector a
set i x (V n t)
 | i < 0 = error "Indice malo"
 | otherwise = (V n (setTree t i x n))
  where
   setTree (Node izq der) i x n
    | i < (div n 2) = (Node (setTree izq i x (div n 2)) der)
	| otherwise = (Node izq (setTree der (i - (div n 2) x (div n 2))
   setTree (Unif _) i x 1 = Unif x
   setTree (Unif y) i x n
    | i < (div n 2) = simplify (setTree (Unif y) i x (div n 2) (Unif y))
	| otherwise = simplify (Unif y) (setTree (Unif y)  (i - (div n 2)) x (div n 2))
...
mapVector f (V tam tree) = (V tam (mapVector' f tree))

mapVector' :: (Eq b) => (a -> b) -> Tree a -> Tree b
mapVector' f (Unif x) = (Unif (f x))
mapVector' f (Node lt rt) = simplify (mapVector' f lt) (mapVector' f rt)
